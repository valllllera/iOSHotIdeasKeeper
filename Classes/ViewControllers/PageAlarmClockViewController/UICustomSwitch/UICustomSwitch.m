...
{
	if ((self=[super initWithFrame:CGRectMake(rect.origin.x,rect.origin.y,70,34)]))
	{
...
-(void)awakeFromNib
{
	...
	[self setMinimumTrackImage:[UIImage imageNamed:@"switch_on.png"] forState:UIControlStateNormal];
	[self setMaximumTrackImage:[UIImage imageNamed:@"switch_off.png"] forState:UIControlStateNormal];
	...
	self.clippingView = [[UIView alloc] initWithFrame:CGRectMake(3,6,64,33)];
	...
	{
		leftLabelText = @"";		// use helvetica lowercase L to be a 1. 
	}
	
	self.leftLabel = [[UILabel alloc] init];
	self.leftLabel.frame = CGRectMake(0, 0, 48, 33);
	...
	if ([rightLabelText length] == 1)
	{
		rightLabelText = @"O";	// use helvetica uppercase o to be a 0. 
	}
	
	self.rightLabel = [[UILabel alloc] init];
	self.rightLabel.frame = CGRectMake(95, 0, 33, 23);
...
-(void)setTintColor:(UIColor*)color
{
	if (color != tintColor)
	{
		...
		[self setMinimumTrackImage:[self image:[UIImage imageNamed:@"switch_on.png"] tintedWithColor:tintColor] forState:UIControlStateNormal];
	}
...
//    self.activeClock.idx = [NSString stringWithFormat:@"%@",clock.idx];
   ...
   // clock.active = [clock.active];
	...
	{
		self.value = 1.0;
        self.activeClock.active = @"1";
        NSString *query = [NSString stringWithFormat:@"update AlarmClock set active = %@ where id = '%@'",self.activeClock.active ,self.activeClock.idx];
